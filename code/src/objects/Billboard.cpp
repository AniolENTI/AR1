...
Billboard::Billboard(glm::vec3 topRight, glm::vec3 topLeft, glm::vec3 botLeft, glm::vec3 botRight, char* path)
{
    //GENERATE BUFFERS
    {
		float vertices[]{
		...
		program->compileAndAttachShader("shaders/Billboard.vert", GL_VERTEX_SHADER);
		program->compileAndAttachShader("shaders/Billboard.geom", GL_GEOMETRY_SHADER);
		program->compileAndAttachShader("shaders/Billboard.frag", GL_FRAGMENT_SHADER);
...
void Billboard::Render()
{
    ...
    glm::mat4 objMat = glm::translate(glm::mat4(), glm::vec3(0.0f, 0.0f, -1.0f));
    ...
    glUniformMatrix4fv(
        program->getUniform("CameraProj"),
        1, GL_FALSE, glm::value_ptr(cam._projection)
    );
    glUniform3f(
        program->getUniform("CameraPos"),
...
void Billboard::SetTransforms(CameraTransforms cam)
{
	this->cam = cam;
}